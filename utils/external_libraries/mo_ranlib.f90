...
! Modular version of the original RANLIB, see references inside particular functions 
...
!    0.0 < BB.
!
!    Output, real GENBET, a beta random variate.
!
  implicit none
  ...
  real k1
  real k2
  real, parameter :: log4 = 1.3862943611198906188E+00
  real, parameter :: log5 = 1.6094379124341003746E+00
  real r
!  real r4_exp
!  real r4_uni_01
  real s
...
!    0.0 < DF.
!
!    Output, real GENCHI, a random deviate from the distribution.
!
  implicit none
...
!
!    Output, real GENEXP, a random deviate from the distribution.
!
  implicit none
...
!    0.0 < DFD.
!
!    Output, real GENF, a random deviate from the distribution.
!
  implicit none
...
!
!    Output, real GENGAM, a random deviate from the distribution.
!
  implicit none
...
!    0.0 <= XNONC.
!
!    Output, real GENNCH, a random deviate from the distribution.
!
  implicit none
...
!    0.0 <= XNONC.
!
!    Output, real GENNF, a random deviate from the distribution.
!
  implicit none
...
!
!    Output, real GENNOR, a random deviate from the distribution.
!
  implicit none
...
!  Parameters:
!
!    Input/output, integer IARRAY(N), an array to be permuted.
!
...
!
!    Output, real GENUNF, a random deviate from the distribution.
!
  implicit none
...
!
!    Output, integer IGNNBN, a random deviate from 
!    the distribution.
!
  implicit none
...
!  real snorm
  ...
  real, parameter :: a2 = -0.2500068E+00
  real, parameter :: a3 =  0.2000118E+00
  real, parameter :: a4 = -0.1661269E+00
  real, parameter :: a5 =  0.1421878E+00
  real, parameter :: a6 = -0.1384794E+00
  real, parameter :: a7 =  0.1250060E+00
  real b1
  real b2
  real c
  real c0
...
!
!    Output, integer IGNUIN, a random deviate from 
!    the distribution.
!
  implicit none
...
!
!    Output, integer LENNOB, the length of the string to the last
!    nonblank.
!
  implicit none
...
!
!    Output, real R4_EXP, the value of exp ( X ).
!
  implicit none

  real r4_exp
  real, parameter :: r4_huge = 1.0E+30
  real, parameter :: r4_log_max = +69.0776E+00
  real, parameter :: r4_log_min = -69.0776E+00
  real value
  real x
...
!
!    Output, real R4_EXPONENTIAL_SAMPLE, a sample of the PDF.
!
  implicit none
...
!
!    Output, real R4VEC_COVAR, the covariance of the vectors.
!
  implicit none
  ...
  x_average = sum ( x(1:n) ) / real ( n )
  y_average = sum ( y(1:n) ) / real ( n )
...
!  Parameters:
!
!    Input, real ( kind = rk8 ) LAMBDA, the parameter of the PDF.
!
!    Output, real ( kind = rk8 ) R8_EXPONENTIAL_SAMPLE, a sample of the PDF.
!
  implicit none

! Used functions
!
!  real ( kind = rk8 ) r8_uni_01

  integer, parameter :: rk8 = kind ( 1.0D+00 )

  real ( kind = rk8 ) lambda
  real ( kind = rk8 ) r
  real ( kind = rk8 ) r8_exponential_sample
...
!  Parameters:
!
!    Input, real ( kind = rk8 ) X(N), Y(N), the two vectors.
!
!    Input, integer N, the dimension of the two vectors.
!
!    Output, real ( kind = rk8 ) R8VEC_COVAR, the covariance of the vectors.
!
  implicit none
  ...
  integer i
  real ( kind = rk8 ) r8vec_covar
  real ( kind = rk8 ) value
  real ( kind = rk8 ) x(n)
  real ( kind = rk8 ) x_average
  real ( kind = rk8 ) y(n)
  real ( kind = rk8 ) y_average

  x_average = sum ( x(1:n) ) / real ( n, kind = rk8 )
  y_average = sum ( y(1:n) ) / real ( n, kind = rk8 )
  ...
  r8vec_covar = value / real ( n - 1, kind = rk8 )
...
!  Parameters:
!
!    Output, real SEXPO, a random deviate from the standard
...
!  real r4_uni_01
  ...
  real si
  real sgamma
  real, parameter :: sqrt32 = 5.656854E+00
  real t
  real u
  real v
  real w
