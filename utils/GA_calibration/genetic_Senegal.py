...
class member(object):
    ...
    def mutate(self,pars,igen,params,rmethod=1,bounce=True):
        ...
        for par in pars:
            ...
            if (par["tol"]>0) and (np.random.uniform()<mrate):
                ...
                # Make sure not out of range. If so, then bounce back into interval
                if bounce:
                    if val < par["min"]:
                        val = 2*par["min"] - val
                    if val > par["max"]:
                ...
                if not bounce:
                # Make sure not out of range. If so, then clip back to min or max
                    val=max(val,par["min"])
                    val=min(val,par["max"])
...
class ensemble:
    ...
    def generation(self,pars,best,params,igen):
      ...
      #  all_ens.append2(self,best) # create long list of best and ens models with deepcopy.
        ...
        all_ens=ensemble(params["nens"])
      #  all_ens.append2(self)
        for i in range(len(self.mem)):
            all_ens.mem[i]=deepcopy(self.mem[i])
      ...
      #  idx=heapq.nlargest(params["nbest"],range(params["nens"]+params["nbest"]),list_fitness.__getitem__)
        idx=heapq.nlargest(params["nbest"],range(params["nens"]),list_fitness.__getitem__)
...
def main(params):
  ...
  #  from tensor_to_csv import tflog2pandas as to_csv
    ...
    #===========================================================================================#
    #
    seed_Senegal = 12345*params["trial"]
    np.random.seed(seed_Senegal) # place a fixed number in brackets for reproducable results
    iparfile=model.init()
    print(iparfile)
    ...
    # 4. Run the model member and calculate its skill
        logging.info("====================================")
        logging.info(f"Running model generation {(igen+1):.0f}")
        logging.info("====================================")
        ...
        if params["lparallel"]:
            ...
            pool.starmap(model.run,([imem,mlab,ens.mem[imem].vals,data_loc] for imem in range(params["nens"]))) # initerr
            pool.close()
            pool.join()
            pool=multiprocessing.Pool(processes=ncore)
            ...
            for imem in range(params["nens"]):
                model.run(imem,mlab,ens.mem[imem].vals,data_loc)
